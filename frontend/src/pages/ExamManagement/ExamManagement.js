...
const ExamManagement = () => {
  ...
  const [isAvailableStudentsModal, setIsAvailableStudentsModal] =
    useState(false);
  ...
  const fetchExams = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/api/teacher/exams/subject/${subjectId}`,
        {
          headers: {
  ...
  const handleShowAvailableStudents = async () => {
    ...
    try {
      ...
      const enrolledStudentIds = new Set(
        enrolledStudentsRes.data.map((student) => student.id)
      );
  ...
  const handleAddStudent = async (studentId) => {
    try {
      ...
      message.error(error.response?.data?.message || "Thêm học sinh thất bại");
    }
  };
    ...
    <div className="exam-management">
      ...
      >
        ...
        {loadingAvailableStudents ? (
          ...
          <List
            dataSource={availableStudents.filter(
              (student) =>
                student.fullname
                  .toLowerCase()
          ...
          <List
            dataSource={studentList.filter(
              (student) =>
                student.fullname
                  .toLowerCase()
                  .includes(searchStudentList.toLowerCase()) ||
                student.username
                  .toLowerCase()
                  .includes(searchStudentList.toLowerCase()) ||
                student.email
