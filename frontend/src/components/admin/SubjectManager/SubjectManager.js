          ...
          getFilteredSubjects().map((subject) => (
            <div key={subject.id} className="subject-card">
              <div className="subject-header">
                <h4>{subject.name}</h4>
                <span
                  className={`status-badge ${getStatusClass(subject.status)}`}
                >
      ...
      >
        <Spin spinning={teacherLoading} tip="Đang tải danh sách giáo viên...">
          <Row gutter={[16, 16]}>
            <Col span={12}>
              <Card
                title={`Giáo viên đã gán (${subjectTeachers.length})`}
                size="small"
              >
                <List
      ...
      >
        <Spin spinning={studentLoading} tip="Đang tải danh sách học sinh...">
          <Row gutter={[16, 16]}>
            <Col span={12}>
              <Card
                title={`Học sinh đã gán (${subjectStudents.length})`}
                size="small"
              >
                <List
          ...
          <div>
            <Descriptions title="Thông tin cơ bản" bordered column={2}>
              ...
              <Descriptions.Item label="Trạng thái">
                <Tag
                  color={
                    selectedSubject.status === 1
                      ? "green"
                      : selectedSubject.status === 0
                      ? "orange"
                      : "red"
                  }
                >
                  ...
                  <Statistic
                    ...
                    value={
                      subjectStudents.length > 0
                        ? (
                            subjectTeachers.length / subjectStudents.length
                          ).toFixed(2)
                        : 0
                    }
